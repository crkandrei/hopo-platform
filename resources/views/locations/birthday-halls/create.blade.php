...
@section('title', 'Adaugă sală')
@section('page-title', 'Adaugă sală')
...
<div class="space-y-6">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 mb-2">Adaugă sală</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ route('locations.birthday-halls.store', $location) }}">
            @csrf
            <div class="space-y-4">
                <div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">{{ old('description') }}</textarea>
                </div>
                <div>
                    ...
                    <input type="number" name="capacity" id="capacity" value="{{ old('capacity', 20) }}" min="1" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <select name="booking_mode" id="booking_mode" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                        <option value="slots" {{ old('booking_mode', 'slots') === 'slots' ? 'selected' : '' }}>Pe sloturi (intervale fixe)</option>
                        <option value="flexible" {{ old('booking_mode') === 'flexible' ? 'selected' : '' }}>Orar liber (orice oră)</option>
                    </select>
